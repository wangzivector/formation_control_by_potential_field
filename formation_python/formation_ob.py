...
class formation:
    def __init__(self):
        ...
        # initial position
        self.initial_pos = (self.target*0.2 + np.random.random_sample(np.shape(self.target))) + 2
        ...
        self.shape_ind = 0
        self.time_stamp = [x*self.dt for x in range(0, int((self.time-0)/self.dt))]
    ...
    def formation_cal(self):
        for index_t in self.time_stamp:
            ...
            F_r = F_r*np.tile(bool_sm, [1, 2]) + F_r_norm_max*np.tile(bool_bi, [1, 2])
            ...
            f_s = f_s*np.tile(bool_sm, [1, 2]) + f_s_norm_max*np.tile(bool_bi, [1, 2])
            f_s[np.isnan(f_s)] = 0
            f_a = F_r + f_s
