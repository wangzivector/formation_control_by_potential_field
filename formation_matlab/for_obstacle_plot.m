function F_o = for_obstacle_plot(cen_o, rect_o, pos, vel)
%     F_o = zeros(size(pos));
...
%     
%     Fs_max = 1; norm_F = sum(factors.^2,2).^(0.5); bool_bi = norm_F>Fs_max; bool_sm = norm_F<=Fs_max; 
...
% 
%    % plot the result
%     plot([x0 - v1, x0 - v1, x0 + v1, x0 + v1,x0 - v1], [y0-v2,y0+v2,y0+v2,y0-v2,y0-v2], '-');
%     axis equal;
%      theta=0:pi/40:2*pi;
%      xc = (A_sq/barrier)^(-0.5) * cos(theta) + x0;
%      yc = (B_sq/barrier)^(-0.5) * sin(theta) + y0;
%     plot(xc,yc,'-');
%      theta=0:pi/40:2*pi;
%      xc = (A_sq)^(-0.5) * cos(theta) + x0;
%      yc = (B_sq)^(-0.5) * sin(theta) + y0;
%      plot(xc,yc,'-');
%     
%     F_o_d = F_o_i + pos;
    vel_d = vel + pos;
    scatter(pos(:,1), pos(:,2), 40, 'o');
     for ind = 1:size(pos,1)
%         plot([pos(ind,1),F_o_d(ind,1)], [pos(ind,2),F_o_d(ind,2)], '-*');
        plot([pos(ind,1),vel_d(ind,1)], [pos(ind,2),vel_d(ind,2)], '--');
     end
...
%     scatter(pos(:,1), pos(:,2), 40, 'o');
%      for ind = 1:size(x)
%         plot([pos(ind,1),F_o_d(ind,1)], [pos(ind,2),F_o_d(ind,2)], 'k-*');
%      end
%      
end